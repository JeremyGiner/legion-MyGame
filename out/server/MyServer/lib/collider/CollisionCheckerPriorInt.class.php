...
class collider_CollisionCheckerPriorInt {
	public function __construct(){}
	static $_bOb = false;
	static $oCollisionEvent;
	static function check($oShape1, $oVelocity1, $oShape2, $oVelocity2) {
		$res = collider_CollisionCheckerPriorInt::_check($oShape1, $oVelocity1, $oShape2, $oVelocity2);
		if($res !== null) {
	...
	static function _check($oShapeA, $oVelocity1, $oShapeB, $oVelocity2) {
		if(Std::is($oShapeA, _hx_qtype("space.Vector2i"))) {
			if(Std::is($oShapeB, _hx_qtype("space.IAlignedAxisBoxi"))) {
				return (isset(collider_CollisionCheckerPriorInt::$_vectorAABB_check) ? collider_CollisionCheckerPriorInt::$_vectorAABB_check: array("collider_CollisionCheckerPriorInt", "_vectorAABB_check"));
			}
		}
		if(Std::is($oShapeA, _hx_qtype("space.IAlignedAxisBoxi"))) {
			if(Std::is($oShapeB, _hx_qtype("space.IAlignedAxisBoxi"))) {
				return (isset(collider_CollisionCheckerPriorInt::$_AABB_check) ? collider_CollisionCheckerPriorInt::$_AABB_check: array("collider_CollisionCheckerPriorInt", "_AABB_check"));
			}
	...
	static function _AABB_check($oBox1, $oVelocity1, $oBox2, $oVelocity2) {
		...
		if(collider_CollisionCheckerPost::axisCollision_get($oBox1->left_get(), $oBox1->right_get(), $oBox2->left_get(), $oBox2->right_get()) > 0) {
			...
			if($oVelocity1->x > 0) {
				$fTime = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oBox1->right_get(), $oVelocity1->x, $oBox2->left_get(), 0);
				if($fTime !== null && $fTime >= 0 && $fTime <= 1) {
					$fXCollisionTime = $fTime;
				}
			} else {
				$fTime1 = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oBox1->left_get(), $oVelocity1->x, $oBox2->right_get(), 0);
				if($fTime1 !== null && $fTime1 >= 0 && $fTime1 <= 1) {
					$fXCollisionTime = $fTime1;
				}
			}
		}
		...
		if(collider_CollisionCheckerPost::axisCollision_get($oBox1->bottom_get(), $oBox1->top_get(), $oBox2->bottom_get(), $oBox2->top_get()) > 0) {
			...
			if($oVelocity1->y > 0) {
				$fTime2 = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oBox1->top_get(), $oVelocity1->y, $oBox2->bottom_get(), 0);
				if($fTime2 !== null && $fTime2 >= 0 && $fTime2 <= 1) {
					$fYCollisionTime = $fTime2;
				}
			} else {
				$fTime3 = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oBox1->bottom_get(), $oVelocity1->y, $oBox2->top_get(), 0);
				if($fTime3 !== null && $fTime3 >= 0 && $fTime3 <= 1) {
					$fYCollisionTime = $fTime3;
				}
			}
		}
		...
		if($fXCollisionTime > $fYCollisionTime) {
			$oNormal = new space_Vector2i(1, 0);
		} else {
			$oNormal = new space_Vector2i(0, 1);
		}
	...
	static function _vectorAABB_check($oPoint1, $oVelocity1, $oBox2, $oVelocity2) {
		...
		if(collider_CollisionCheckerPost::axisCollision2_get($oPoint1->x, $oPoint1->x, $oBox2->left_get(), $oBox2->right_get()) === true) {
			...
			if($oVelocity1->x > 0) {
				$fTime = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oPoint1->x, $oVelocity1->x, $oBox2->left_get(), 0);
				if($fTime !== null && $fTime >= 0 && $fTime <= 1) {
					$fXCollisionTime = $fTime;
				}
			} else {
				$fTime1 = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oPoint1->x, $oVelocity1->x, $oBox2->right_get(), 0);
				if($fTime1 !== null && $fTime1 >= 0 && $fTime1 <= 1) {
					$fXCollisionTime = $fTime1;
				}
			}
		...
		if(collider_CollisionCheckerPost::axisCollision2_get($oPoint1->y, $oPoint1->y, $oBox2->bottom_get(), $oBox2->top_get()) === true) {
			...
			if($oVelocity1->y > 0) {
				$fTime2 = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oPoint1->y, $oVelocity1->y, $oBox2->bottom_get(), 0);
				if($fTime2 !== null && $fTime2 >= 0 && $fTime2 <= 1) {
					$fYCollisionTime = $fTime2;
				}
			} else {
				$fTime3 = collider_CollisionCheckerPriorInt::axisCollisionTime_get($oPoint1->y, $oVelocity1->y, $oBox2->top_get(), 0);
				if($fTime3 !== null && $fTime3 >= 0 && $fTime3 <= 1) {
					$fYCollisionTime = $fTime3;
				}
			}
	...
	function __toString() { return 'collider.CollisionCheckerPriorInt'; }
}
