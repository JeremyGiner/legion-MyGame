...
class websocket_php_SocketDistant extends websocket_php_Socket {
	...
	public function process() {
		...
		if(_hx_equal($iByteN, false)) {
			...
			if($error !== 0) {
				haxe_Log::trace("[ERROR]:socket error:" . _hx_string_or_null(socket_strerror($error)), _hx_anonymous(array("fileName" => "SocketDistant.hx", "lineNumber" => 63, "className" => "websocket.php.SocketDistant", "methodName" => "process")));
			}
			return;
		}
		if($iByteN === 0) {
		...
		if(strlen($this->_sInBuffer) === 0) {
			haxe_Log::trace("[WARNING]:did not close proper way", _hx_anonymous(array("fileName" => "SocketDistant.hx", "lineNumber" => 73, "className" => "websocket.php.SocketDistant", "methodName" => "process")));
			$this->_close();
			return;
		}
	...
	public function _message_handle() {
		haxe_Log::trace("[NOTICE]:SocketDistant:message receive : " . _hx_string_or_null($this->_sInBuffer), _hx_anonymous(array("fileName" => "SocketDistant.hx", "lineNumber" => 105, "className" => "websocket.php.SocketDistant", "methodName" => "_message_handle")));
	...
	function __toString() { return 'websocket.php.SocketDistant'; }
}
